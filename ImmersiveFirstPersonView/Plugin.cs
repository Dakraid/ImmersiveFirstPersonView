...
namespace IFPV
{
    ...
    public sealed class IFPVPlugin : Plugin
    {
        ...
        private void init()
        {
            ...
            Events.OnUpdateCamera.Register(
                e =>
                {
                    if ( this.CameraMain == null || !this.CameraMain.IsInitialized )
                    {
            ...
            Events.OnUpdatedPlayerHeadtrack.Register(
                e =>
                {
                    if ( this.CameraMain != null && this.CameraMain.IsInitialized )
                    {
            ...
            Events.OnShadowCullingBegin.Register(
                e =>
                {
                    if ( this.CameraMain == null || !this.CameraMain.IsInitialized || !this.CameraMain.IsEnabled || !this.Settings.SeparateShadowCulling )
                    {
                        return;
                    }
            ...
            Events.OnShadowCullingEnd.Register(
                e =>
                {
                    if ( this.CameraMain != null && this.CameraMain.IsInitialized && this.CameraMain.IsEnabled && this.Settings.SeparateShadowCulling )
                    {
            ...
            Events.OnWeaponFireProjectilePosition.Register(
                e =>
                {
                    if ( this.CameraMain == null || !this.CameraMain.IsInitialized )
                    {
            ...
            this.InstallHook("player update animation", 39445, 0x97, 5, "E8", ctx => { Interlocked.Increment(ref this._is_spine); }, ctx => { Interlocked.Decrement(ref this._is_spine); });

            this.InstallHook("player control inc counter", 41259, 0, 5, "48 89 5C 24 08", ctx => { Interlocked.Increment(ref this._is_spine); });

            this.InstallHook("player controls dec counter", 41259, 0x241, 5, "48 83 C4 30 5F", ctx => { Interlocked.Decrement(ref this._is_spine); });

            this.InstallHook(
                "player movement controller type",
                40937,
                0x2EA,
                6,
                "FF 90 88 03 00 00",
