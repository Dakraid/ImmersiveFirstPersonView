...
namespace IFPV.States
{
    internal sealed class Default : CameraState
    {
        ...
        internal override bool Check(CameraUpdate update) { return update.CameraMain.IsEnabled; }
        ...
        internal override void Update(CameraUpdate update)
        {
            ...
            {
                if (_lastCollidedRestrict == null)
                    _lastCollidedRestrict = update.Values.RestrictDown.AddModifier(
...

            {
                ...
                if (Settings.Instance.TryFixLeftRightMovementClipping != 0.0f &&
                    !Stack.CameraMain.WasUsingFirstPersonArms                 && ((moveType = Moving._move_dir) == 2 || moveType == 6))
                {
                    var swimming = false;
                    ...
                    if (!swimming)
                    {
                        ...
                        if (_look_downoffset_ratio_leftrightmove < dr)
                            IncLeftRightMoveFix(dr);
                        else if (_look_downoffset_ratio_leftrightmove > dr)
                            DecLeftRightMoveFix(dr);
                    }
                    else if (_look_downoffset_ratio_leftrightmove != 0.0f) { DecLeftRightMoveFix(0.0f); }
                }
                else if (_look_downoffset_ratio_leftrightmove != 0.0f) { DecLeftRightMoveFix(0.0f); }
            }
        }
        ...
        private void UpdateNearClip(CameraUpdate update, double? y)
        {
            ...
            if (_nearClip == null || _lastNearClip != nc)
            {
                if (_nearClip != null)
                {
                    _nearClip.Remove();
                ...
                _nearClip     = update.Values.NearClip.AddModifier(this, CameraValueModifier.ModifierTypes.Set, nc, false);
                _lastNearClip = nc;
            }
        }
    }
