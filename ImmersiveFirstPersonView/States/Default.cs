    ...
    internal sealed class Default : CameraState
    {
        ...
        internal override void Update(CameraUpdate update)
        {
...

            {
                ...
                if ( Settings.Instance.TryFixLeftRightMovementClipping != 0.0f && !this.Stack.CameraMain.WasUsingFirstPersonArms && ((moveType = Moving._move_dir) == 2 || moveType == 6) )
                {
                    var swimming = update.Target.Actor != null && (Memory.ReadUInt32(update.Target.Actor.Address + 0xC0) & 0x400) != 0;
        ...
        private void UpdateNearClip(CameraUpdate update, double? y)
        {
            ...
            var ncNormal = interior ? Settings.Instance.NearClipInteriorDefault : Settings.Instance.NearClipExteriorDefault;
