...
void* crtti__RTDynamicCast(void* _obj, unsigned int _target, void* _module)
{
    ...
    __try
    {
        ...
        crtti__RTTICompleteObjectLocator* loc = *((
            crtti__RTTICompleteObjectLocator**)(
            vtable - sizeof(Pointer)));
        do
        {
            if (loc->offset != 0)
            {
                obj -= loc->offset;
                vtable = *((Pointer*)obj);
                loc = *((crtti__RTTICompleteObjectLocator**)(
                    vtable - sizeof(Pointer)));
                continue;
            }
        ...
        {
            crtti__RTTIClassHierarchyDescriptor* hierarchy = (
                crtti__RTTIClassHierarchyDescriptor*)(
                module + loc->pClassDescriptor);
            if (hierarchy->pBaseClassArray != 0)
            {
                crtti__RTTIBaseClassArray* arr = (
                    crtti__RTTIBaseClassArray*)(
                    module + hierarchy->pBaseClassArray);
                for (int i = 0; i < hierarchy->numBaseClasses; i
                     ++)
                {
                    crtti__RTTIBaseClassDescriptor* base = (
                        crtti__RTTIBaseClassDescriptor*)(module + arr->
                        arrayOfBaseClassDescriptors[i]);
...
void crtti__Explore(void* _obj, Pointer* baseObj, Pointer* data,
                    int maxDataCount, void* _module)
{
    Pointer module = (Pointer)_module;
    Pointer obj = (Pointer)_obj;
