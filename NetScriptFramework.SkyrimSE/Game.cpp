...
void CrashLogModListWriter__Write(NetScriptFramework::CrashLogEventArgs ^e)
{
	...
	mods->Sort(
		gcnew System::Comparison<TESFile ^
		>(CrashLogModListWriter__Sort));
	e->Output->AppendLine();
...
{
	_map = gcnew System::Collections::Generic::Dictionary<
		System::String ^, System::Collections::Generic::List<BGSKeyword
			^> ^>();
...
void NiObjectLoadParameters::_RequestModelDirect(NiObjectLoadParameters ^p)
{
	...
	NetScriptFramework::Memory::WritePointer(
		p->_alloc->Address, System::IntPtr::Zero, false);
	NetScriptFramework::Memory::WriteInt32(p->_alloc->Address + 0x10, 0,
	                                       false);
	NetScriptFramework::Memory::WriteInt32(p->_alloc->Address + 0x14, 3,
	                                       false);
	NetScriptFramework::Memory::WriteInt32(p->_alloc->Address + 0x18,
	                                       0x1010001, false);

	auto str = NetScriptFramework::Memory::AllocateString(
		p->FileName, false);
	try {
		result = (System::Int32)(
			NetScriptFramework::Memory::InvokeCdecl(
				__VIDS::VID74039.Value, str->Address,
				p->_alloc->Address, p->_alloc->Address + 0x10).
			ToInt64() & 0xFF);
	} finally {
		delete str;
	}
...
void NiObjectLoadParameters::_RequestModelAsync(NiObjectLoadParameters ^p)
{
	...
	try {
		result = (System::Int32)(
			NetScriptFramework::Memory::InvokeCdecl(
				__VIDS::VID74038.Value, str->Address,
				p->_alloc->Address, p->_alloc->Address + 0x10,
				0).ToInt64() & 0xFF);
	} finally {
		delete str;
...
{
	System::Collections::Generic::List<NiObjectLoadParameters ^> ^ls =
		nullptr;
	System::Threading::Monitor::Enter(_locker);
	try {
		if (_queued->Count == 0)
		...
		ls = _queued;
		_queued = gcnew System::Collections::Generic::List<
...
bool NiObjectLoadParameters::Process()
{
	auto ptr = NetScriptFramework::Memory::ReadPointer(
		_alloc->Address, false);
	if (ptr != System::IntPtr::Zero) {
		auto prototypePtr = NetScriptFramework::Memory::ReadPointer(
			ptr + 0x28, false);
		if (prototypePtr == System::IntPtr::Zero)
			return false;

		auto obj = NetScriptFramework::MemoryObject::FromAddress<
			NiObject ^>(prototypePtr);
		if (obj != nullptr) {
...
void NiObjectLoadParameters::Cleanup()
{
	...
	if (_alloc != nullptr) {
		auto ptr = NetScriptFramework::Memory::ReadPointer(
			_alloc->Address, false);
		if (ptr != System::IntPtr::Zero) {
			auto db = NetScriptFramework::Memory::ReadPointer(
				          __VIDS::VID514145.Value,
				          false) + 0xD0;
			NetScriptFramework::Memory::InvokeCdecl(
				__VIDS::VID11275.Value, db, ptr);
		}
...
RayCastResult ^RayCastResult::GetClosestResult(array<float> ^source,
                                               System::Collections::Generic::List
                                               <RayCastResult ^> ^results,
                                               ... array<NiAVObject ^> ^ignore)
{
	if (source == nullptr)
	...
	if (source->Length != 3)
		throw gcnew
			System::ArgumentOutOfRangeException("source.Length");
			...
			if (checkIgnore) {
				...
				if (obj != nullptr) {
					...
					for (int i = 0; i < ignore->Length; i++
						...
						auto io = (
							NetScriptFramework::IMemoryObject
							^)ignore[i];
						if (io != nullptr && io->Equals(
							    obj)) {
							ok = false;
							break;
						}
...
System::Collections::Generic::List<RayCastResult ^> ^_Havok::RayCast(
	RayCastParameters ^p)
{
	if (p == nullptr)
	...
	auto havokWorldPtr = NetScriptFramework::Memory::InvokeCdecl(
		__VIDS::VID18536.Value, p->Cell->Cast<TESObjectCELL ^>());
	...
	*((int *)(args + 0x24)) = 0;
	// this is sometimes set in specific actor or object ref cases
	...
	*((void **)(args + 0xA8)) = &collector;
	...
	auto vtable = NetScriptFramework::Memory::ReadPointer(
		havokWorldPtr, false);
	auto func51 = NetScriptFramework::Memory::ReadPointer(
		vtable + 0x198, false);
	System::IntPtr callfn = NetScriptFramework::Memory::InvokeCdecl(
		func51, havokWorldPtr, System::IntPtr(args));
	...
	while (n != 0) {
		...
		{
			obj = NetScriptFramework::Memory::InvokeCdecl(
				__VIDS::VID76160.Value, obj);
		}
		r->_obj = obj;
...
TESForm ^TESForm::LookupFormFromFile(System::UInt32 formId,
                                     System::String ^fileName)
{
	...
	if (dataHandler == nullptr)
		return nullptr;
	auto alloc =
		NetScriptFramework::Memory::AllocateString(fileName, false);
	auto alloc2 = NetScriptFramework::Memory::Allocate(0x10, 0, false);
	try {
		auto filePtr = NetScriptFramework::Memory::InvokeCdecl(
			__VIDS::VID13632.Value, dataHandler->Address,
			alloc->Address);
		if (filePtr == System::IntPtr::Zero)
			return nullptr;
		if ((MCH::u(NetScriptFramework::Memory::InvokeCdecl(
			     __VIDS::VID13882.Value, filePtr)) & 0xFF) == 0)
			return nullptr;
		alloc2->Zero();
		NetScriptFramework::Memory::WriteUInt32(
			alloc2->Address, formId, false);
		NetScriptFramework::Memory::InvokeCdecl(
			__VIDS::VID13915.Value, filePtr, alloc2->Address);
		formId = NetScriptFramework::Memory::ReadUInt32(
			alloc2->Address, false);
...
System::Collections::Generic::List<RayCastResult ^> ^TESObjectCELL::RayCast(
	RayCastParameters ^p)
{
	if (p == nullptr)
		throw gcnew System::ArgumentNullException("p");
...
void SkyrimSEGame::Initialize()
{
	...
	Events::OnFrame->Register(
		gcnew NetScriptFramework::Event<FrameEventArgs ^
		>::EventHandler(NiObjectLoadParameters__OnFrame), 0, 0,
		NetScriptFramework::EventRegistrationFlags::None);

	Events::OnMainMenu->Register(
		gcnew NetScriptFramework::Event<MainMenuEventArgs ^
		>::EventHandler(KeywordCache__Initialize), 0, 1,
		NetScriptFramework::EventRegistrationFlags::None);

	NetScriptFramework::CrashLog::OnAfterWrite->Register(
		gcnew NetScriptFramework::Event<CrashLogEventArgs ^
		>::EventHandler(CrashLogModListWriter__Write), 0, 0,
		NetScriptFramework::EventRegistrationFlags::None);

	NetScriptFramework::CrashLog::OnBeforeWrite->Register(
		gcnew NetScriptFramework::Event<CrashLogEventArgs ^
		>::EventHandler(DontWriteCrashLogOnExit__Write), 0, 0,
		NetScriptFramework::EventRegistrationFlags::None);
}

void SkyrimSEGame::RegisterTypes()
{
	auto modulePtr = NetScriptFramework::Main::GetMainTargetedModule()->
		BaseAddress;
	System::Reflection::Assembly ^assembly = nullptr;
	System::IO::FileInfo ^file = gcnew System::IO::FileInfo(
		NetScriptFramework::Main::FrameworkPath +
		"\\NetScriptFramework.SkyrimSE.Implementations.dll");
	if (!file->Exists)
		throw gcnew System::IO::FileNotFoundException(
			"The implementations file was not found!",
			NetScriptFramework::Main::FrameworkPath +
			"\\NetScriptFramework.SkyrimSE.Implementations.dll");
	NetScriptFramework::Loader::Load(file, assembly);
	auto types = assembly->GetTypes();
	for each (System::Type ^t in types) {
		if (t->Name != "NetScriptFramework_SkyrimSE_TypeRegistrations")
			continue;
		...
		for each (System::Reflection::MethodInfo ^m in methods) {
			if (!m->IsStatic || m->Name != "RegisterTypes" || m->
			    GetParameters()->Length != 0 || m->ReturnType !=
			    System::Collections::Generic::List<System::Tuple<
				    System::Int32, System::Type ^,
				    System::UInt64> ^>::typeid)
				continue;
			auto ls = safe_cast<System::Collections::Generic::List<
				System::Tuple<
					System::Int32, System::Type ^,
					System::UInt64> ^> ^>(m->Invoke(
				nullptr, gcnew array<System::Object ^>(0)));
			for each (auto t in ls) {
				if (t->Item1 >= 0)
					this->RegisterImplementationType(
						(System::UInt32)t->Item1,
						t->Item2, t->Item3);
				else
					this->RegisterInterfaceType(
						(System::UInt32)(-t->Item1),
						t->Item2, t->Item3);
			}
		}
