...
public ref class KeywordCache sealed {
	static System::Collections::Generic::Dictionary<
		System::String ^, System::Collections::Generic::List<BGSKeyword
			^> ^> ^_map;
	static initonly System::Collections::Generic::List<BGSKeyword ^> ^_empty
		= gcnew System::Collections::Generic::List<BGSKeyword ^>();
...
public:
	...
	static System::Collections::Generic::IReadOnlyList<BGSKeyword ^> ^Get(
		System::String ^text)
	{
		if (text == nullptr)
			return _empty;
...
public:
	...
	{
		System::Action<NiObjectLoadParameters ^> ^get()
		{
			return _callback;
		}

		void set(System::Action<NiObjectLoadParameters ^> ^value)
		{
			_callback = value;
		}
	}
	...
	{
		System::Collections::Generic::IReadOnlyList<NiObject ^> ^get()
		{
			return _result;
		}
	}
...
private:
	...
	System::Collections::Generic::List<NiObject ^> ^_result = gcnew
		System::Collections::Generic::List<NiObject ^>();
	MemoryAllocation ^_alloc = nullptr;
	bool _success = false;
	static System::Object ^_locker = gcnew System::Object();
	static System::Collections::Generic::List<NiObjectLoadParameters ^> ^
...
public:
	...
	ObjectRefHolder(TESObjectREFR ^obj)
	{
		...
		_allocated = false;
		_obj = obj;
		auto alloc = NetScriptFramework::Memory::Allocate(
			0x10, 0, false);
		NetScriptFramework::Memory::WriteUInt32(
			alloc->Address, 0, false);
		try {
			NetScriptFramework::Memory::InvokeCdecl(
				__VIDS::VID12192.Value, alloc->Address, objptr);
			_value = NetScriptFramework::Memory::ReadUInt32(
				alloc->Address, false);
		} finally {
			if (alloc != nullptr)
				delete alloc;
	...
	ObjectRefHolder(System::UInt32 refId)
	{
		...
		if (refId != 0) {
			auto alloc = NetScriptFramework::Memory::Allocate(
				0x20, 0, false);
			NetScriptFramework::Memory::WriteUInt32(
				alloc->Address, refId, false);
			NetScriptFramework::Memory::WritePointer(
				alloc->Address + 0x10, System::IntPtr::Zero,
				false);
			try {
				NetScriptFramework::Memory::InvokeCdecl(
					__VIDS::VID16828.Value, alloc->Address,
					alloc->Address + 0x10);
				System::IntPtr result =
					NetScriptFramework::Memory::ReadPointer(
						alloc->Address + 0x10, false);
				if (result != System::IntPtr::Zero)
					_obj =
						NetScriptFramework::MemoryObject::FromAddress
...
public:
	...
	property NiAVObject ^Object
	{
		NiAVObject ^get()
		{
			return NetScriptFramework::MemoryObject::FromAddress<
				NiAVObject ^>(_obj);
		}
	}
...
public:
	...
	/// <returns></returns>
	static RayCastResult ^GetClosestResult(array<float> ^source,
...
public:
	...
	StringRefHolder(System::String ^text)
	{
		...
		_alloc = NetScriptFramework::Memory::Allocate(
			0x10 + bytes->Length + 8, 0, false);
		{
			NetScriptFramework::Memory::WriteBytes(
				_alloc->Address + 0x10, bytes, false);
			NetScriptFramework::Memory::WriteUInt32(
				_alloc->Address + 0x10 + bytes->Length, 0,
				false);
			NetScriptFramework::Memory::InvokeCdecl(
				__VIDS::VID67819.Value, _alloc->Address,
				_alloc->Address + 0x10);
		}
	}
	...
	property System::IntPtr Value
	{
		System::IntPtr __clrcall get()
		{
			...
			return NetScriptFramework::Memory::ReadPointer(
	...
	property System::String ^Text
	{
		System::String ^ __clrcall get()
		{
			...
			return NetScriptFramework::Memory::ReadString(
				ptr, false, false);
		}
	}
...
protected:
	void Free() override
	{
		if (_alloc != nullptr) {
			NetScriptFramework::Memory::InvokeCdecl(
				__VIDS::VID67822.Value, _alloc->Address);
...
public:
	static System::Collections::Generic::List<RayCastResult ^> ^RayCast(
		RayCastParameters ^p);
};
}
}
